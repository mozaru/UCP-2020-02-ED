...
char *obterElemento(TLista l, int pos);
void atribuirElemento(TLista *l, int pos, TInfo info);
...
void inserirNoInicio(TLista *l,TInfo info)
{
   ...
   else
   {
      ...
      strcpy((*l).v[novo].info,info);                       //coloca a informação no novo no
...
void inserirOrdenado(TLista *l,TInfo info)
{
   ...
   else
   {
        ...
        int aux = (*l).inicio;
        int ant = -1;
        while( aux != -1 && strcmp((*l).v[aux].info,info)<0 )
        {
            ant = aux;
            aux = (*l).v[aux].prox;
        }
...
int main(void)
{
  ...
  for(i=0;i<obterQtd(l);i++)
    printf("%s ", obterElemento(l,i));
  printf("\n");
  
  while (scanf("%s",&nome)>0)
  {
     if (existe(l,nome))
     {
        removerElemento(&l,nome);
